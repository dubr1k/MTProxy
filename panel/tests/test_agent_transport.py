...
async def test_real_tls_poll_binds_san_serial_and_fingerprint_then_records_result(tmp_path):
    ca, server_key, server_cert, node_key, node_cert, metadata = issue_fixture(tmp_path, "example-node-02")
    store = FleetStore(tmp_path / "panel.sqlite3")
    store.register_node("example-node-02", "Example Region 2", {})
    store.bind_certificate("example-node-02", metadata)
    queued = store.enqueue(
        "example-node-02", "disable-alice-01", "telemt.user.disable", {"username": "alice"},
    ...
    try:
        async with httpx.AsyncClient(
            ...
            response = await client.get("/agent/v1/nodes/example-node-02/commands/next")
            ...
            result = await client.put(
                f"/agent/v1/nodes/example-node-02/commands/{queued['command_id']}/result",
                json={
        ...
        assert store.commands("example-node-02")[0]["status"] == "succeeded"
        node = store.node("example-node-02")
...
async def test_tls_rejects_unknown_ca_and_route_rejects_certificate_for_other_node(tmp_path):
    ...
    bad_key, bad_cert, _ = other.issue_node("example-node-02", days=2)
    ...
    try:
        with pytest.raises(httpx.TransportError):
            async with httpx.AsyncClient(base_url=base_url, verify=mtls_context(ca.cert_path, bad_cert, bad_key), trust_env=False) as client:
                await client.get("/agent/v1/nodes/example-node-02/commands/next")
...
async def test_revocation_and_request_body_bound_fail_closed(tmp_path):
    ...
    store.register_node("example-node-02", "Example Region 2", {})
    store.bind_certificate("example-node-02", metadata)
    store.revoke_certificate("example-node-02", metadata["serial"])
    ...
    try:
        async with httpx.AsyncClient(base_url=base_url, verify=mtls_context(ca.cert_path, node_cert, node_key), trust_env=False) as client:
            assert (await client.get("/agent/v1/nodes/example-node-02/commands/next")).status_code == 403
            oversized = await client.put(
                "/agent/v1/nodes/example-node-02/commands/00000000-0000-0000-0000-000000000000/result",
                content=b"x" * 257,
            )
...
async def test_agent_client_retries_result_from_durable_outbox_without_reexecution(tmp_path):
    ...
    store.bind_certificate("example-node-02", metadata)
    store.enqueue(
        "example-node-02", "disable-alice-01", "telemt.user.disable", {"username": "alice"}, "rev-1",
    ...
    agent = NodeAgent("example-node-02", journal, Executor())
    client = AgentTransportClient(
        node_id="example-node-02", central_url=base_url, cert=node_cert, key=node_key,
