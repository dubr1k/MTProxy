...
function proxyLink(value){
  try{const url=new URL(value);const allowed=(url.protocol==='tg:'&&url.hostname==='proxy'&&(url.pathname===''||url.pathname==='/'))||(url.protocol==='https:'&&['t.me','telegram.me'].includes(url.hostname)&&url.pathname==='/proxy');const keys=[...url.searchParams.keys()];const server=url.searchParams.getAll('server'),port=url.searchParams.getAll('port'),secret=url.searchParams.getAll('secret');if(!allowed||url.hash||keys.length!==3||new Set(keys).size!==3||server.length!==1||port.length!==1||secret.length!==1||!/^[A-Za-z0-9.-]{1,253}$/.test(server[0])||!/^[0-9]+$/.test(port[0])||Number(port[0])<1||Number(port[0])>65535||!/^[0-9A-Fa-f]{32,512}$/.test(secret[0]))throw new Error();return value}catch{throw new Error('Сервис вернул некорректную ссылку подключения')}}
...
async function renderDashboard(generation=navigationGeneration){
  ...
  if(generation!==navigationGeneration||state.view!=='dashboard')return;
