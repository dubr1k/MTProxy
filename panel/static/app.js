...
async function renderDashboard(generation=navigationGeneration){
  ...
  const system=document.querySelector('.system-mini');system.classList.toggle('degraded',!ready);system.querySelector('b').textContent=ready?'Telemt работает':'Telemt недоступен';system.querySelector('small').textContent=ready?'Сервис отвечает':'Проверьте состояние сервиса';
  view.innerHTML=`<div class="metric-grid">
    ...
    <section class="panel-card"><div class="panel-head"><h2>Состояние системы</h2><span>обновлено сейчас</span></div><div class="health-hero ${ready?'':'degraded'}"><span class="health-orb">${ready?icon('check'):'!'}</span><span><b>${ready?'Прокси доступен':'Прокси недоступен'}</b><small>${ready?'Telemt health check пройден':'Telemt health check не пройден'}</small></span></div><div class="service-list"><div class="service-row ${ready?'':'degraded'}"><i></i><span><b>Telemt proxy</b><small>MTProto / FakeTLS</small></span><em>${ready?'healthy':'unavailable'}</em></div><div class="service-row ${location.protocol==='https:'?'':'degraded'}"><i></i><span><b>Web-панель</b><small>${location.protocol==='https:'?'HTTPS · защищённое соединение':'HTTP · соединение не защищено'}</small></span><em>${location.protocol==='https:'?'secure':'insecure'}</em></div><div class="service-row"><i></i><span><b>Пользователи</b><small>${active} активных ключей</small></span><em>${blocked?'есть блокировки':'норма'}</em></div></div></section>
...
function naiveRow(user){const enabled=user.enabled===true;return `<div class="data-row naive-grid" data-naive-name="${esc(user.username)}"><div class="identity"><span class="user-glyph naive-glyph">${esc(initials(user.username))}</span><span><b>${esc(user.username)}</b><small>HTTPS · HTTP/2 CONNECT</small></span></div><div class="cell"><span class="status-pill ${enabled?'active':'blocked'}"><i></i>${enabled?'Активен':'Отключён'}</span></div><div class="cell"><span class="protocol-pill">Naive</span></div><div class="row-actions">${state.me.role!=='viewer'?`<button class="action-button share" data-naive-action="access" data-user="${esc(user.username)}">Конфигурация</button><button class="action-button" data-naive-action="${enabled?'disable':'enable'}" data-user="${esc(user.username)}">${enabled?'Отключить':'Включить'}</button><button class="action-button" data-naive-action="rotate" data-user="${esc(user.username)}">Новый пароль</button><button class="action-button danger-text" data-naive-action="delete" data-user="${esc(user.username)}">Удалить</button>`:'<span class="cell">Только просмотр</span>'}</div></div>`}
...
function syncNaiveCreateButton(){const input=document.querySelector('#new-naive-user'),button=document.querySelector('#create-naive');button.disabled=!input.value||!input.checkValidity()}
...
async function naiveAction(action,username,button){
  try{
    ...
    setBusy(button,true);
    if(action==='delete')await api(`/api/naive/users/${encodeURIComponent(username)}`,{method:'DELETE'});else{const data=await api(`/api/naive/users/${encodeURIComponent(username)}/${action}`,{method:'POST'});if(action==='rotate')await revealNaiveToken(data.reveal_token,username)}
    toast(action==='delete'?'Naive-доступ удалён':action==='disable'?'Naive-доступ отключён':action==='enable'?'Naive-доступ включён':'Пароль обновлён');await renderNaive();
  }catch(error){toast(error.message,'error')}finally{setBusy(button,false)}
}

async function init(){
  try{state.me=await api('/api/auth/me');document.querySelector('#profile-name').textContent=state.me.username;document.querySelector('#profile-role').textContent=roleNames[state.me.role]||state.me.role;document.querySelector('#avatar').textContent=initials(state.me.username);document.querySelectorAll('[data-view="naive"]').forEach(x=>x.hidden=state.me.features?.naive!==true);document.querySelectorAll('.owner-only').forEach(x=>x.hidden=state.me.role!=='owner');document.querySelectorAll('.audit-nav').forEach(x=>x.hidden=false);await navigate('dashboard')}catch(error){errorState(error)}
}

if(view){
document.querySelectorAll('[data-view]').forEach(button=>button.addEventListener('click',()=>navigate(button.dataset.view)));
document.querySelector('#add').addEventListener('click',()=>state.view==='admins'?openAdminModal():state.view==='naive'?openNaiveModal():openUserModal());
...
document.querySelector('#new-naive-user').addEventListener('input',syncNaiveCreateButton);
...
document.querySelector('#create-naive').addEventListener('click',async event=>{const input=document.querySelector('#new-naive-user'),error=document.querySelector('#naive-error'),username=input.value;error.textContent='';if(!input.reportValidity())return;try{setBusy(event.currentTarget,true,'Применяем…');const data=await api('/api/naive/users',{method:'POST',body:JSON.stringify({username})});document.querySelector('#naive-modal').close();await revealNaiveToken(data.reveal_token,username);toast('Naive-доступ создан');await refreshNaive();paintNaive()}catch(e){error.textContent=e.message}finally{setBusy(event.currentTarget,false)}});
