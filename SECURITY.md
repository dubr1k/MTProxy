...
The repository test/Compose/image gates are maintained. Ubuntu 24.04 QEMU full lifecycle and production Mieru/fleet deployment are pending; see [VALIDATION.md](docs/VALIDATION.md). Legacy `install_mtproxy.sh` has a different host/systemd/firewall model and must not share a listener without a specific audit.
