...
The panel also contains a durable fleet registry and typed per-node command queue. Its read-only Fleet view and authenticated APIs are documented in [FLEET.en.md](FLEET.en.md). Remote agent enrollment and polling are intentionally absent until per-node mTLS is available; queued fleet commands do not leave the central database in this release.
