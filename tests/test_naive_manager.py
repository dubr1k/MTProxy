...
def test_bootstrap_transactionally_migrates_missing_cover_host(tmp_path):
    hooks = Hooks()
    service = manager(tmp_path, hooks)
    service.bootstrap()
    config_before = service.caddyfile.read_text().replace(
        "            hosts naive.example.com\n", "",
    )
    service.caddyfile.write_text(config_before)
    state_before = service.state_file.read_bytes()
    backups_before = set(service.backup_dir.iterdir())
    hooks.reloads = hooks.probes = 0
    ...
    rendered = service.caddyfile.read_text()
    forward = rendered.split("forward_proxy {\n", 1)[1].split("        }\n", 1)[0]
    assert forward.count("hosts naive.example.com") == 1
    assert service.state_file.read_bytes() == state_before
    assert hooks.reloads == 1
    assert hooks.probes == 1
    assert not (service.state_file.parent / "transaction.json").exists()
    new_backups = set(service.backup_dir.iterdir()) - backups_before
    config_backup = next(path for path in new_backups if path.name.endswith(".Caddyfile"))
    state_backup = next(path for path in new_backups if path.name.endswith(".users.json"))
    assert config_backup.read_text() == config_before
    assert state_backup.read_bytes() == state_before

    service.bootstrap()
    assert set(service.backup_dir.iterdir()) == backups_before | new_backups
    assert hooks.reloads == 1
    assert hooks.probes == 1
...
@pytest.mark.parametrize(
    "hosts",
    ["hosts other.example.com", "hosts naive.example.com\n            hosts naive.example.com"],
)
def test_bootstrap_rejects_conflicting_or_duplicate_cover_hosts_without_mutation(
    tmp_path, hosts,
):
    hooks = Hooks()
    service = manager(tmp_path, hooks)
    service.bootstrap()
    service.caddyfile.write_text(
        service.caddyfile.read_text().replace("hosts naive.example.com", hosts),
    )
    config_before = service.caddyfile.read_bytes()
    state_before = service.state_file.read_bytes()
    backups_before = set(service.backup_dir.iterdir())
    ...
    with pytest.raises(
        ManagerConflict, match="forward_proxy hosts must match the managed public host",
    ):
        service.bootstrap()

    assert service.caddyfile.read_bytes() == config_before
    assert service.state_file.read_bytes() == state_before
    assert set(service.backup_dir.iterdir()) == backups_before
    assert hooks.reloads == 0
    assert hooks.probes == 0
    assert not (service.state_file.parent / "transaction.json").exists()
...
@pytest.mark.parametrize(
    "directive",
    [
        "hosts naive.example.com # cover bypass",
        "hosts    naive.example.com",
        "hosts\tnaive.example.com",
    ],
)
def test_bootstrap_keeps_semantically_matching_cover_host_as_noop(
    tmp_path, directive,
):
    hooks = Hooks()
    service = manager(tmp_path, hooks)
    ...
    assert service.caddyfile.read_text() == config_before
    assert service.state_file.read_bytes() == state_before
    assert set(service.backup_dir.iterdir()) == backups_before
    assert hooks.reloads == 0
    assert hooks.probes == 0
...
def test_bootstrap_rejects_cover_host_conflict_before_accounting_migration(tmp_path):
    hooks = Hooks()
    service = manager(tmp_path, hooks)
    service.bootstrap()
    lines = service.caddyfile.read_text().splitlines()
    ...
    legacy = "\n".join(lines[:begin] + lines[end + 1:]) + "\n"
    legacy = legacy.replace("hosts naive.example.com", "hosts other.example.com")
    service.caddyfile.write_text(legacy)
    state_before = service.state_file.read_bytes()
    backups_before = set(service.backup_dir.iterdir())
    hooks.reloads = hooks.probes = 0
    ...
    assert service.caddyfile.read_text() == legacy
    assert service.state_file.read_bytes() == state_before
    assert set(service.backup_dir.iterdir()) == backups_before
    assert hooks.reloads == 0
    assert hooks.probes == 0
...
def test_cover_host_migration_rolls_back_config_and_state_when_probe_fails(tmp_path):
    hooks = Hooks()
    service = manager(tmp_path, hooks)
    service.bootstrap()
    config_before = service.caddyfile.read_text().replace(
    ...
    assert service.caddyfile.read_text() == config_before
    assert service.state_file.read_bytes() == state_before
    assert hooks.reloads == 2
    assert hooks.probes == 2
    assert "hosts naive.example.com" in hooks.reload_snapshots[0]
    assert hooks.reload_snapshots[1] == config_before
    assert not (service.state_file.parent / "transaction.json").exists()



@pytest.mark.parametrize("fault_phase", ["prepared", "files_replaced", "rollback_pending"])
def test_cover_host_migration_recovers_each_durable_phase_and_retries(
    tmp_path, fault_phase,
):
    hooks = Hooks()
    service = manager(tmp_path, hooks)
    service.bootstrap()
    old_config = service.caddyfile.read_text().replace(
        "            hosts naive.example.com\n", "",
    )
    migrated_config = old_config.replace(
        "            upstream socks5://127.0.0.1:40000\n",
        "            upstream socks5://127.0.0.1:40000\n"
        "            hosts naive.example.com\n",
    )
    old_state = service.state_file.read_bytes()
    config_backup = service.backup_dir / "cover-recovery.Caddyfile"
    state_backup = service.backup_dir / "cover-recovery.users.json"
    config_backup.write_text(old_config)
    state_backup.write_bytes(old_state)
    service.caddyfile.write_text(
        old_config if fault_phase == "prepared" else migrated_config,
    )
    (service.state_file.parent / "transaction.json").write_text(json.dumps({
        "version": 1,
        "phase": fault_phase,
        "config_backup": config_backup.name,
        "state_backup": state_backup.name,
        "operation": "cover_host_migration",
    }))
    hooks.reloads = hooks.probes = 0
    hooks.reload_snapshots.clear()
    ...
    assert recovered.caddyfile.read_text().count("hosts naive.example.com") == 1
    assert hooks.reload_snapshots[0] == old_config
    assert "hosts naive.example.com" in hooks.reload_snapshots[1]
...
def test_migration_operation_is_rejected_for_initial_bootstrap_journal(tmp_path):
    hooks = Hooks()
    service = manager(tmp_path, hooks)
    service.bootstrap()
    config_backup = next(service.backup_dir.glob("*.Caddyfile"))
