...
Учёт Naive — это полезные байты завершённых туннелей без TLS/IP-накладных расходов, а не жёсткая квота и не платёжный счётчик. При сбое manager не удаляйте `transaction.json`, paired backups, `-wal` или `-shm`.
...
- перезагружает или перезапускает только соответствующую службу и проверяет `is-active`;
- сохраняет предыдущий бинарник/override и при ошибке восстанавливает его;
