...
def check(socket_path: Path | str, token_path: Path | str, *, timeout: float = 3) -> bool:
    try:
...
def main() -> None:
    socket_path = os.getenv("NAIVE_MANAGER_SOCKET", "/run/naive-manager/manager.sock")
    token_path = os.getenv("NAIVE_MANAGER_TOKEN_FILE", "/run/secrets/naive-manager-token")
